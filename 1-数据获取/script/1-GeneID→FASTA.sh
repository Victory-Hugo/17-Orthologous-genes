...
# 默认配置
INPUT="/mnt/f/OneDrive/文档（科研）/脚本/Download/17-Orthologous-genes/1-数据获取/conf/lolD.txt" #? 每行一个GeneID
OUTDIR="/mnt/f/OneDrive/文档（科研）/脚本/Download/17-Orthologous-genes/1-数据获取/data/lolD/"
...
retry_esearch() {
  ...
  local retry_count=0
  local result=""
  local timeout=15  # 单次请求超时15秒
  
  while (( retry_count < max_retries )); do
    # 使用 timeout 命令设置超时
    result=$(timeout $timeout esearch -db "$db" -query "$query" 2>/dev/null)
    local exit_code=$?
    
    if [[ $exit_code -eq 124 ]]; then
      # 超时
      echo "    esearch 超时（>${timeout}s）..." >&2
      ((retry_count++))
      sleep $((retry_count * 2))
      continue
    fi
...
safe_download_protein_fasta() {
  ...
  local max_retries=3
  local retry_count=0
  local download_timeout=30  # 下载超时30秒
  ...
  while (( retry_count < max_retries )); do
    ...
    # 执行下载命令（带超时控制）
    if timeout $download_timeout bash -c "retry_esearch \"$gene_id\" \"gene\" | elink -target protein | efetch -format fasta" > "$output_file" 2>/dev/null; then
...
safe_download_nucleotide_fasta() {
  ...
  while (( retry_count < max_retries )); do
    ...
    # 使用 timeout 避免网络卡住
    if timeout $download_timeout bash -c "retry_esearch \"$gene_id\" \"gene\" | elink -target nuccore | efetch -format fasta" > "$output_file" 2>/dev/null; then
      # 检查文件是否有实际内容（大于 100 字节且包含FASTA格式）
      if [[ -s "$output_file" ]] && (( $(stat -c%s "$output_file") > 100 )) && grep -q "^>" "$output_file"; then
        return 0
      fi
...
# 修复：使用 -0 分隔符和 printf 避免 xargs 参数冲突
{
    local index=0
    for gene_id in "${GENE_IDS[@]}"; do
        ((index++))
        printf '%d\0%s\0' "$index" "$gene_id"
    done
} | xargs -0 -P"$PARALLEL_JOBS" -n2 bash -c '
    current=$1
    gene_id=$2
    total='"${#GENE_IDS[@]}"'
