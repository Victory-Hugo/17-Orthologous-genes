...
CONFIG_PATH="${BASE_DIR}/conf/hyphy_purifying_selection.json"
JOBS="${JOBS:-1}"
FORCE="false"
...
DATASET_CMD=("$PYTHON_BIN" "${BASE_DIR}/python/hyphy_purifying_selection.py" --config "$CONFIG_PATH" --list-datasets)
RUN_CMD=("$PYTHON_BIN" "${BASE_DIR}/python/hyphy_purifying_selection.py" --config "$CONFIG_PATH" --log-dir "$LOG_DIR" --tmp-dir "$TMP_DIR")
if [[ "$FORCE" == "true" ]]; then
  RUN_CMD+=(--force)
fi
RUN_CMD+=(--dataset)
